...
use crate::{
  ...
  models::{AuthSession, InitUserRequest, LoginRequest, RegisterRequest, SuccessResponse, UpdateUserRequest, UserRecord},
};
...
async fn register(State(state): State<AppState>, Json(payload): Json<RegisterRequest>) -> ApiResult<Json<AuthSession>> {
...
async fn login(State(state): State<AppState>, Json(payload): Json<LoginRequest>) -> ApiResult<Json<AuthSession>> {
...
async fn init_user(State(state): State<AppState>, Json(payload): Json<InitUserRequest>) -> ApiResult<Json<AuthSession>> {
  let username = payload.username.trim();
  ...
  if let Some(existing_user) = runtime.users.values().find(|user| user.username == username) {
...
async fn complete_onboarding(Path(user_id): Path<String>, State(state): State<AppState>) -> Json<SuccessResponse> {
  let mut runtime = state.runtime.write().expect("runtime lock poisoned");
