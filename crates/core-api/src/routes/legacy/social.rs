...
use crate::{
  ...
  models::{
    FriendListItemRecord, FriendRequestRecord, FriendshipRecord, SendFriendRequestPayload, ShakePreviewCharacterRecord,
    ShakeResultRecord, SuccessResponse, TriggerScenePayload, UserScopedRequest,
  },
};
...
async fn accept_request(
  ...
  if let Some(existing) = runtime
    .friendships
    .values()
    .find(|friendship| friendship.user_id == payload.user_id && friendship.character_id == request.character_id)
    .cloned()
  {
...
async fn get_friends(
  ...
  let items = runtime
    ...
    .filter_map(|friendship| {
      runtime.characters.get(&friendship.character_id).map(|character| FriendListItemRecord {
...
async fn shake(
  ...
  Json(Some(ShakeResultRecord {
    ...
    greeting: format!("Hello, I'm {}. We just met in Hidden World.", character.name),
  }))
}
...
async fn trigger_scene(
  ...
  let request = build_friend_request(
    ...
    Some(format!("Hello, I'm {}. We crossed paths in {}.", character.name, payload.scene)),
  );
