...
async fn find_one(Path(id): Path<String>, State(state): State<AppState>) -> ApiResult<Json<CharacterRecord>> {
  let runtime = state.runtime.read().expect("runtime lock poisoned");
...
async fn create(State(state): State<AppState>, Json(payload): Json<CharacterPatch>) -> Json<CharacterRecord> {
