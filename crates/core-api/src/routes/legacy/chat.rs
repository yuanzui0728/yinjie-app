...
use crate::{
  ...
  models::{
    ...
    GetOrCreateConversationPayload, GroupMemberRecord, GroupMessageRecord, GroupRecord, MessageRecord,
    SendGroupMessagePayload, UserScopedRequest,
  },
};
...
fn groups_router() -> Router<AppState> {
  Router::new()
    ...
    .route("/:id/members", get(get_group_members).post(add_group_member))
    .route("/:id/messages", get(get_group_messages).post(send_group_message))
}
...
async fn get_conversations(
        ...
        .filter(|message| {
          message.sender_type == "character"
            && match conversation.last_read_at.as_ref() {
              Some(last_read) => parse_timestamp(&message.created_at) > parse_timestamp(last_read),
              None => true,
            }
        })
        .count();
  ...
  items.sort_by(|left, right| parse_timestamp(&right.updated_at).cmp(&parse_timestamp(&left.updated_at)));
  Json(items)
}
...
async fn get_group(Path(id): Path<String>, State(state): State<AppState>) -> ApiResult<Json<GroupRecord>> {
  let runtime = state.runtime.read().expect("runtime lock poisoned");
  let group = runtime
    .groups
    .get(&id)
...
async fn get_group_members(Path(id): Path<String>, State(state): State<AppState>) -> Json<Vec<GroupMemberRecord>> {
  let runtime = state.runtime.read().expect("runtime lock poisoned");
...
async fn add_group_member(
  ...
  if let Some(existing) = members.iter().find(|member| member.member_id == payload.member_id) {
    return Json(existing.clone());
  }
...
async fn get_group_messages(Path(id): Path<String>, State(state): State<AppState>) -> Json<Vec<GroupMessageRecord>> {
  let runtime = state.runtime.read().expect("runtime lock poisoned");
  let mut messages = runtime.group_messages.get(&id).cloned().unwrap_or_default();
  messages.sort_by(|left, right| parse_timestamp(&right.created_at).cmp(&parse_timestamp(&left.created_at)));
  Json(messages)
}
